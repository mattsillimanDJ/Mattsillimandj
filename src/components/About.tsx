...
export function About() {
  // IMPORTANT: this must match src/public/images/about-bg.png
  const FALLBACK_BG = '/images/about-bg.png';
  ...
  useEffect(() => {
    const loadContent = async () => {
      try {
        ...
        const aboutContent = data?.content?.find(
          (item: any) => item?.key === 'cms_content_about'
        );
    ...
    const loadImages = async () => {
      try {
        ...
        const data = await response.json();
        const aboutBg = data?.images?.find(
          (item: any) => item?.key === 'cms_image_aboutBg'
        );

        const candidate = aboutBg?.value;
        const isValid =
        ...
        setBgImageUrl(isValid ? candidate : FALLBACK_BG);
      } catch (err) {
  ...
  return (
    <section id="about" className="min-h-screen flex items-center py-24 px-6 relative">
      ...
      <div
        ...
        onError={() => setBgImageUrl(FALLBACK_BG)}
      />

      {/* Content */}
      <div className="max-w-4xl mx-auto relative z-10">
        <h2 className="text-5xl md:text-6xl mb-12 tracking-tight">
          {content.title}
        </h2>
